...
from downstream.retrieval import train_retrieval, inference_retrieval
from downstream.matching import train_matching, inference_matching
from datasets import SimCLRDataset, RetrievalDataset, MatchingDataset
...
def train_retrieval_stage(pretrained_model, args):
    logger = setup_logger(args['retrieval_save_dir'], 'retrieval')
    
    transform = get_standard_transforms(size=224)
    train_dataset = RetrievalDataset(args['retrieval_train_dir'], transform, 'train')
    val_dataset = RetrievalDataset(args['retrieval_val_dir'], transform, 'val')
    
    model = train_retrieval(pretrained_model, train_dataset, val_dataset, args)
    logger.info("Retrieval training completed")
...
def main():
    args = {
        ...
        'simclr_epochs': 100,
        'retrieval_train_dir': 'data/retrieval_train',
        'retrieval_val_dir': 'data/retrieval_val',
        'retrieval_save_dir': 'checkpoints/retrieval',
    ...
    for key in ['simclr_save_dir', 'retrieval_save_dir', 'matching_save_dir']:
        os.makedirs(args[key], exist_ok=True)
    
    print("Stage 1: Training SimCLR (Contrastive Learning)")
    simclr_model = train_simclr_stage(args)
    
    print("Stage 2: Training Image Retrieval (Downstream Task)")
    retrieval_model = train_retrieval_stage(simclr_model, args)
    
    print("Stage 3: Training Image Matching (Downstream Task)")
